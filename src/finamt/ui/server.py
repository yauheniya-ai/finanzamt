...
---------------------
    ...
    npm install
    npm run build
    cp -r dist/* ../src/finamt/ui/static/
...
def launch(
    ...
    """
    Start the finamt API + UI server.
    ...
    try:
        ...
        print(
            ...
            "        Install the UI extras:  pip install finamt[ui]",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if has_static:
        ...
        print(
            ...
            f"cp -r dist/* src/finamt/ui/static/"
        )
...
def main() -> None:
    ...
    # Always show finamt extraction logs at INFO level so the user can
    # see what the LLM extracted. Use --log-level debug for full output.
    logging.getLogger("finamt").setLevel(
